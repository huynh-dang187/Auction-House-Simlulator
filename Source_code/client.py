...
import base64
import io
import winsound # [MỚI] Thư viện âm thanh
import os       # [MỚI] Để tìm đường dẫn file
...
class AuctionClientGUI(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("BID WARS: FINAL SOUND EDITION")
        ...
        self.tab_view = ctk.CTkTabview(box, width=280, height=300, fg_color="transparent")
        ...
        header = ctk.CTkFrame(self.frame_main, height=60, fg_color=COLOR_CARD, corner_radius=0)
        ...
        # Tabs: Market | Inventory
        self.main_tabs = ctk.CTkTabview(self.frame_main, fg_color="transparent")
        self.main_tabs.pack(fill="both", expand=True, padx=5, pady=0)
        ...
        product_area.pack(fill="both", expand=True, padx=5, pady=10)
        ...
        self.lbl_current_price = ctk.CTkLabel(self.frame_info, text="$0", font=("Impact", 36), text_color="white")
        ...
        def create_btn(txt, color):
            ctk.CTkButton(self.frame_info, text=txt, fg_color=color, text_color="white", height=35, font=("Arial", 12, "bold"), 
        ...
        self.frame_chat = ctk.CTkFrame(self.tab_market, fg_color=COLOR_CARD, height=130, corner_radius=0, border_width=1, border_color="#333")
        self.frame_chat.pack(fill="x", padx=5, pady=(0, 10))
        ...
        self.txt_chat.pack(side="left", fill="both", expand=True, padx=5, pady=5)
        self.txt_chat.configure(state="disabled")
        chat_in = ctk.CTkFrame(self.frame_chat, fg_color="transparent")
        chat_in.pack(side="right", fill="y", padx=5, pady=5)
        self.entry_chat = ctk.CTkEntry(chat_in, placeholder_text="Chat...", width=120, height=120, fg_color="#1a1a1a", border_color="#444", text_color="white")
        self.entry_chat.pack(fill="both", expand=True)
        self.entry_chat.bind('<Return>', lambda e: self.send_chat())

        # === INVENTORY TAB ===
        ctk.CTkButton(self.tab_inventory, text="↻ REFRESH", fg_color="#333", command=self.req_inventory).pack(fill="x", pady=5)
    ...
    def play_sound(self, sound_name):
        try:
            # Tìm file trong folder "sounds"
            path = os.path.join("sounds", f"{sound_name}.wav")
            if os.path.exists(path):
                # SND_ASYNC: Chạy song song không làm đơ phần mềm
    ...
    def connect_server(self, type, u, p, bal=None):
        ...
        try:
            ...
            elif resp.startswith("REG_OK|"): messagebox.showinfo("Success", resp.split("|")[1]); s.close()
            else: messagebox.showerror("Error", resp); s.close()
        except Exception as e: messagebox.showerror("Error", str(e))

    def update_balance_ui(self): self.lbl_balance.configure(text=f"$ {self.balance}")
        ...
        if msg.startswith("START|"):
            ...
            if img_str and img_str != "NO_IMG":
                try:
                    ...
                    ctk_img = ctk.CTkImage(pil_img, size=(250, 250)) 
            ...
            # [ÂM THANH] Phát tiếng khi có người Bid
            self.play_sound("bid")
            ...
            # --- [LOGIC MỚI] HIỆU ỨNG 10 GIÂY CUỐI ---
            if t <= 10: 
                self.lbl_timer.configure(fg_color="#b71c1c", text_color="#ffffff") # Đổi màu Đỏ báo động
                ...
                # Trả lại màu bình thường nếu > 10s (hoặc khi được bù giờ)
                self.lbl_timer.configure(fg_color="black", text_color="white")
            # ------------------------------------------
    ...
    def add_inventory_item(self, name, price, date, img_str):
        card = ctk.CTkFrame(self.scroll_inv, fg_color=COLOR_CARD, corner_radius=0, border_color="#333", border_width=1)
        ...
        if img_str and img_str != "NO_IMG":
            try:
                ...
                ctk.CTkLabel(card, text="", image=ctk_img).pack(side="left", padx=5, pady=5)
            except: pass
        else: ctk.CTkLabel(card, text="[NO IMG]", width=50).pack(side="left", padx=5)
        
        info = ctk.CTkFrame(card, fg_color="transparent")
        info.pack(side="left", fill="both", expand=True)
        ctk.CTkLabel(info, text=name, font=("Arial", 12, "bold"), text_color="white", anchor="w").pack(fill="x")
        ctk.CTkLabel(info, text=f"${price}", font=("Impact", 14), text_color="#4CAF50", anchor="w").pack(fill="x")
        ctk.CTkLabel(card, text=date.split(" ")[0], font=("Arial", 10), text_color="gray").pack(side="right", padx=10)
