...
class AuctionServerGUI(ctk.CTk):
    def __init__(self):
        ...
        self.current_image_data = "NO_IMG" # [MỚI] Lưu trữ ảnh hiện tại để lát nữa lưu vào DB
    ...
    def get_encoded_image(self):
        ...
        try:
            with Image.open(self.selected_image_path) as img:
                img.thumbnail((300, 300)) # Nén nhỏ vừa phải để lưu DB không bị quá nặng
                buffered = io.BytesIO()
    ...
    def start_auction(self):
        ...
        self.current_image_data = self.get_encoded_image() # [MỚI] Lưu vào biến class
                ...
                try:
                    ...
                    while "\n" in buffer:
                        ...
                        if msg.startswith("BID|"): self.process_bid(client_socket, msg)
                            ...
                            self.broadcast(f"CHAT|{username}|{msg.split('|', 1)[1]}")
                        
                        # --- [MỚI] XỬ LÝ LỆNH LẤY INVENTORY ---
                        elif msg.startswith("REQ_INVENTORY"):
                            items = self.db.get_user_inventory(username)
                            # Gửi từng món đồ về: INV_ITEM|Name|Price|Date|ImgData
                            for item in items:
                                # item = (name, price, date, img)
                                send_msg = f"INV_ITEM|{item[0]}|{item[1]}|{item[2]}|{item[3]}\n"
                                client_socket.send(send_msg.encode('utf-8'))
                                time.sleep(0.05) # Delay nhẹ để tránh dính gói tin
                        # --------------------------------------
    ...
    def end_auction(self):
        with self.auction_lock:
            ...
            # --- [MỚI] LƯU VẬT PHẨM VÀO KHO CỦA NGƯỜI THẮNG ---
            if self.highest_bidder != "None":
                ...
                self.log(f"Item saved to {self.highest_bidder}'s inventory.")
            # --------------------------------------------------
