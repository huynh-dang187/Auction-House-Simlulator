...
# --- THEME PURE BLACK ---
COLOR_BG = "#000000"         # Đen tuyệt đối
COLOR_PANEL = "#121212"      # Xám than (cho các khối)
COLOR_BORDER = "#333333"     # Viền nhẹ để phân cách
COLOR_ACCENT = "#ffffff"     # Trắng (Tương phản cao nhất)
COLOR_TEXT_LOG = "#00ff00"   # Xanh lá Terminal cổ điển
...
class AuctionServerGUI(ctk.CTk):
    def __init__(self):
        ...
        # === SIDEBAR ===
        self.sidebar = ctk.CTkFrame(self, width=250, corner_radius=0, fg_color=COLOR_PANEL, border_width=0)
        self.sidebar.grid(row=0, column=0, sticky="nsew")
        
        ctk.CTkLabel(self.sidebar, text="AUCTION\nCONTROL", font=("Arial", 24, "bold"), text_color="white").pack(pady=(40, 40))

        # INPUTS
        self.create_label("ITEM NAME")
        self.entry_item = self.create_entry("Ex: iPhone 16")
        
        self.create_label("START PRICE")
        self.entry_price = self.create_entry("Ex: 1000")
        
        # IMAGE
        self.create_label("IMAGE ASSET")
        ...
        self.card_status = self.create_stat(self.stats_frame, "SYSTEM STATUS", "ONLINE", "#4CAF50") # Green Flat
        self.card_users = self.create_stat(self.stats_frame, "CONNECTIONS", "0", "white")
        self.card_highest = self.create_stat(self.stats_frame, "HIGHEST BID", "$0", "#FFC107") # Amber Flat

        # Log Area (Terminal Style)
        ctk.CTkLabel(self.main_area, text="TERMINAL OUTPUT", font=("Consolas", 12, "bold"), text_color="#555", anchor="w").pack(fill="x", pady=(0,5))
    ...
    def create_label(self, text):
        ctk.CTkLabel(self.sidebar, text=text, font=("Arial", 10, "bold"), text_color="#777", anchor="w").pack(fill="x", padx=20, pady=(10, 2))

    def create_entry(self, ph):
    ...
    def create_stat(self, parent, title, value, color):
        card = ctk.CTkFrame(parent, fg_color=COLOR_PANEL, corner_radius=0, border_width=1, border_color="#222")
        card.pack(side="left", fill="both", expand=True, padx=5)
        ctk.CTkLabel(card, text=title, font=("Arial", 10, "bold"), text_color="gray").pack(pady=(15, 0))
        lbl = ctk.CTkLabel(card, text=value, font=("Arial", 22, "bold"), text_color=color)
        lbl.pack(pady=(0, 15))
        return lbl

    # --- LOGIC (GIỮ NGUYÊN TỪ V6) ---
    ...
    def start_auction(self):
        ...
        self.card_highest.configure(text=f"${self.current_price}")
        ...
        try:
            while True:
                ...
                if cmd == "LOGIN":
                    ...
                    if success:
                        if user in self.client_names.values(): client_socket.send("AUTH_FAIL|Online!".encode('utf-8'))
                        else:
            ...
            self.card_users.configure(text=f"{len(self.clients)}")
